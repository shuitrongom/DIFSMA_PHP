...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        try {
            $stmt = $pdo->prepare('SELECT COALESCE(MAX(orden), 0) + 1 AS next_orden FROM slider_comunica');
            $stmt->execute();
            $nextOrden = (int) $stmt->fetchColumn();

            $stmt = $pdo->prepare(
                'INSERT INTO slider_comunica (imagen_path, orden, activo) VALUES (?, ?, 1)'
            );
...
$stmt = $pdo->query('SELECT * FROM slider_comunica ORDER BY orden ASC');
        ...
        <nav id="sidebar" class="sidebar d-flex flex-column">
            ...
            <ul class="nav flex-column mt-2">
                <?php foreach ($sections as $s): ?>
                    <li class="nav-item">
                        <a class="nav-link<?= $s['file'] === 'slider_comunica.php' ? ' active' : '' ?>" href="<?= htmlspecialchars($s['file']) ?>">
                    ...
                    <div class="col-lg-8">
                        <div class="card">
                            <div class="card-header">
                                <i class="bi bi-images me-1"></i> Imágenes del Slider DIF Comunica
