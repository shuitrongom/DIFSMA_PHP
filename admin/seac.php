...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!csrf_validate($token)) {
        ...
        header('Location: seac' . ($bloqueId > 0 ? "?bloque_id={$bloqueId}" : ''));
        exit;
    }
    ...
    if ($action === 'create_block') {
        ...
        try {
            ...
            if ($s->fetch()) { $_SESSION['flash_message'] = "Ya existe bloque {$anio}."; $_SESSION['flash_type'] = 'warning'; header('Location: seac'); exit; }
    ...
    if ($action === 'add_concepto') {
        ...
        if ($bId <= 0 || $nombre === '') {
            ...
            $_SESSION['flash_type'] = 'warning';
            header("Location: seac?bloque_id={$bId}"); exit;
        }
        try {
    ...
    if ($action === 'delete_pdf') {
        ...
        if ($pdfId<=0) { $_SESSION['flash_message']='ID inválido.'; $_SESSION['flash_type']='danger'; header("Location: seac?bloque_id={$bId}"); exit; }
        $s=$pdo->prepare('SELECT pdf_path FROM seac_pdfs WHERE id=?'); $s->execute([$pdfId]); $row=$s->fetch();
        if (!$row) { $_SESSION['flash_message']='PDF no encontrado.'; $_SESSION['flash_type']='danger'; header("Location: seac?bloque_id={$bId}"); exit; }
        try {
    ...
    if ($action === 'delete_block') {
        $id=(int)($_POST['id']??0);
        if ($id<=0) { $_SESSION['flash_message']='ID inválido.'; $_SESSION['flash_type']='danger'; header('Location: seac'); exit; }
        $s=$pdo->prepare('SELECT id,anio FROM seac_bloques WHERE id=?'); $s->execute([$id]); $bl=$s->fetch();
        if (!$bl) { $_SESSION['flash_message']='Bloque no encontrado.'; $_SESSION['flash_type']='danger'; header('Location: seac'); exit; }
        try {
        ...
        <nav class="navbar navbar-light bg-white shadow-sm px-3">
            ...
            <span class="navbar-brand mb-0 h6">
                <?php if ($currentBloque): ?>
                    <a href="seac" class="text-decoration-none text-muted">SEAC</a>
...
<?php if ($bloqueId > 0 && $currentBloque): ?>
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <form method="POST" action="seac?bloque_id=<?= (int)$currentBloque['id'] ?>" class="row g-2 align-items-end">
...
<?php if ($cell && !empty($cell['pdf_path'])): ?>
                                        ...
                                        <form method="POST" enctype="multipart/form-data" action="seac?bloque_id=<?= (int)$currentBloque['id'] ?>" class="mb-1">
                                        ...
                                        <form method="POST" action="seac?bloque_id=<?= (int)$currentBloque['id'] ?>" onsubmit="return confirm('¿Eliminar este PDF?');">
...
<?php else: ?>
                                        <form method="POST" enctype="multipart/form-data" action="seac?bloque_id=<?= (int)$currentBloque['id'] ?>">
                                            <input type="hidden" name="action" value="upload_pdf">
                                            <input type="hidden" name="bloque_id" value="<?= (int)$currentBloque['id'] ?>">
                                            <input type="hidden" name="concepto_id" value="<?= (int)$c['id'] ?>">
                                            <input type="hidden" name="trimestre" value="<?= $t ?>">
...
<?php endfor; ?>
                                    <td class="text-center">
                                        ...
                                        <form method="POST" action="seac?bloque_id=<?= (int)$currentBloque['id'] ?>" class="d-inline" onsubmit="return confirm('¿Eliminar concepto y sus PDFs?');">
                                ...
                                <div class="modal fade" id="editC<?= (int)$c['id'] ?>" tabindex="-1" aria-hidden="true">
                                    <div class="modal-dialog"><div class="modal-content">
                                        <form method="POST" action="seac?bloque_id=<?= (int)$currentBloque['id'] ?>">
...
<?php elseif ($bloqueId > 0 && !$currentBloque): ?>
            <div class="alert alert-warning"><i class="bi bi-exclamation-triangle me-1"></i> Bloque no encontrado. <a href="seac" class="alert-link">Volver</a>.</div>
...
<?php foreach ($bloques as $bl): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <a href="seac?bloque_id=<?= (int)$bl['id'] ?>" class="btn btn-sm btn-outline-primary"><i class="bi bi-eye"></i> Ver</a>
