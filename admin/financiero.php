...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action==='delete_block') {
        $id=(int)($_POST['id']??0);
        try {
            $sp=$pdo->prepare('SELECT pdf_path FROM fin_conceptos WHERE bloque_id=? AND pdf_path IS NOT NULL AND pdf_path!=""'); $sp->execute([$id]); $pdfs=$sp->fetchAll();
            $pdo->prepare('DELETE FROM fin_bloques WHERE id=?')->execute([$id]);
    ...
    if ($action==='delete_concepto') {
        ...
        try { $s=$pdo->prepare('SELECT pdf_path FROM fin_conceptos WHERE id=?'); $s->execute([$cId]); $old=$s->fetchColumn();
            if ($old&&file_exists(BASE_PATH.'/'.$old)) unlink(BASE_PATH.'/'.$old);
            $pdo->prepare('DELETE FROM fin_conceptos WHERE id=? AND bloque_id=?')->execute([$cId,$bId]);
            $s=$pdo->prepare('SELECT id FROM fin_conceptos WHERE bloque_id=? ORDER BY orden ASC'); $s->execute([$bId]);
            $n=1; $u=$pdo->prepare('UPDATE fin_conceptos SET numero=?,orden=? WHERE id=?');
...
try {
    if ($bloqueId>0) {
        $s=$pdo->prepare('SELECT * FROM fin_bloques WHERE id=?'); $s->execute([$bloqueId]); $currentBloque=$s->fetch();
        if ($currentBloque) {
            $s=$pdo->prepare('SELECT * FROM fin_conceptos WHERE bloque_id=? ORDER BY orden ASC'); $s->execute([$bloqueId]); $conceptos=$s->fetchAll();
        }
    } else {
        $s=$pdo->query('SELECT b.*, (SELECT COUNT(*) FROM fin_conceptos c WHERE c.bloque_id=b.id) AS num_conceptos FROM fin_bloques b ORDER BY b.anio DESC');
        $bloques=$s->fetchAll();
    }
} catch(PDOException $e) { if(defined('APP_DEBUG')&&APP_DEBUG) error_log('FIN: '.$e->getMessage()); }
...
<title>Financiero — Admin DIF</title>
...
<button class="btn btn-sm btn-outline-light d-md-none" id="closeSidebar"><i class="bi bi-x-lg"></i></button>
</div>
<ul class="nav flex-column mt-2">
<?php foreach($sections as $sc): ?>
<li class="nav-item"><a class="nav-link<?=$sc['file']==='financiero.php'?' active':''?>" href="<?=htmlspecialchars($sc['file'])?>"><i class="bi <?=htmlspecialchars($sc['icon'])?>"></i> <?=htmlspecialchars($sc['title'])?></a></li>
<?php endforeach; ?>
</ul>
<div class="mt-auto p-3 border-top border-secondary"><a href="logout.php" class="btn btn-outline-danger btn-sm w-100"><i class="bi bi-box-arrow-right me-1"></i> Cerrar sesión</a></div>
</nav>
...
<h5 class="mb-0"><i class="bi bi-calendar-event me-1"></i> Financiero <?=htmlspecialchars($currentBloque['anio'])?></h5>
...
<div class="card mb-4"><div class="card-header bg-success text-white"><i class="bi bi-plus-circle me-1"></i> Agregar concepto</div>
...
<?php else:?>
<a href="../<?=htmlspecialchars($c['pdf_path'])?>" target="_blank" class="btn btn-sm btn-outline-info"><i class="bi bi-eye"></i></a>
<button class="btn btn-sm btn-outline-warning" data-bs-toggle="modal" data-bs-target="#uP<?=(int)$c['id']?>"><i class="bi bi-arrow-repeat"></i></button>
...
<div class="card-header"><i class="bi bi-currency-dollar me-1"></i> Bloques Financiero <span class="badge bg-secondary ms-1"><?=count($bloques)?></span></div>
...
<thead class="table-light"><tr><th>Año</th><th>Conceptos</th><th style="width:120px">Acciones</th></tr></thead><tbody>
