...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_concepto') {
        ...
        try {
            ...
            $pdo->prepare('INSERT INTO conac_conceptos (bloque_id,numero,nombre,orden) VALUES (?,?,?,?)')->execute([$bId,$num,$nombre,$num]);
    ...
    if ($action === 'delete_concepto') {
        ...
        try {
            ...
            $pdo->prepare('DELETE FROM conac_pdfs WHERE concepto_id=? AND bloque_id=?')->execute([$cId,$bId]);
            $pdo->prepare('DELETE FROM conac_conceptos WHERE id=? AND bloque_id=?')->execute([$cId,$bId]);
            foreach ($pdfs as $p) { $f = BASE_PATH.'/'.$p['pdf_path']; if (file_exists($f)) unlink($f); }
            // Renumber
            $s = $pdo->prepare('SELECT id FROM conac_conceptos WHERE bloque_id=? ORDER BY orden ASC'); $s->execute([$bId]);
            $n=1; $u=$pdo->prepare('UPDATE conac_conceptos SET numero=?,orden=? WHERE id=?');
    ...
    if ($action === 'upload_pdf') {
        ...
        try {
            ...
            if ($ex) {
                ...
                $pdo->prepare('UPDATE conac_pdfs SET pdf_path=? WHERE id=?')->execute([$upload['path'],$ex['id']]);
            } else {
                $pdo->prepare('INSERT INTO conac_pdfs (bloque_id,concepto_id,trimestre,pdf_path) VALUES (?,?,?,?)')->execute([$bId,$cId,$trim,$upload['path']]);
            }
...
try {
    if ($bloqueId > 0) {
        $s = $pdo->prepare('SELECT * FROM conac_bloques WHERE id=?'); $s->execute([$bloqueId]);
        $currentBloque = $s->fetch();
        if ($currentBloque) {
            $s = $pdo->prepare('SELECT * FROM conac_conceptos WHERE bloque_id=? ORDER BY orden ASC');
            $s->execute([$bloqueId]); $conceptos = $s->fetchAll();
            $s = $pdo->prepare('SELECT id,concepto_id,trimestre,pdf_path FROM conac_pdfs WHERE bloque_id=?');
            $s->execute([$bloqueId]);
            while ($r = $s->fetch()) { $pdfsMap[(int)$r['concepto_id']][(int)$r['trimestre']] = ['id'=>(int)$r['id'],'pdf_path'=>$r['pdf_path']]; }
        }
    } else {
        $s = $pdo->query('SELECT b.*,COUNT(p.id) AS num_pdfs FROM conac_bloques b LEFT JOIN conac_pdfs p ON p.bloque_id=b.id AND p.pdf_path IS NOT NULL AND p.pdf_path!="" GROUP BY b.id ORDER BY b.anio DESC');
        $bloques = $s->fetchAll();
    }
} catch (PDOException $e) {
    if (defined('APP_DEBUG') && APP_DEBUG) { error_log('CONAC query error: ' . $e->getMessage()); }
}
...
<head>
    ...
    <title>CONAC — Panel de Administración DIF</title>
    <link rel="icon" href="../img/favicon-32x32.png" sizes="35x35">
    <link rel="stylesheet" href="../css/bootstrap.min.css">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css">
    <link rel="stylesheet" href="../css/admin.css">
    <style>.conac-cell{min-width:160px;vertical-align:middle}</style>
</head>
<body>
<div class="d-flex">
    <nav id="sidebar" class="sidebar d-flex flex-column">
        ...
        <ul class="nav flex-column mt-2">
            ...
            <li class="nav-item"><a class="nav-link<?= $sc['file']==='conac.php'?' active':'' ?>" href="<?= htmlspecialchars($sc['file']) ?>"><i class="bi <?= htmlspecialchars($sc['icon']) ?>"></i> <?= htmlspecialchars($sc['title']) ?></a></li>
            <?php endforeach; ?>
        </ul>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover align-middle mb-0">
                            <thead class="table-light">
                                <tr>
                                    <th>Concepto</th>
                                    <th class="conac-cell text-center">1er Trimestre</th>
                                    <th class="conac-cell text-center">2do Trimestre</th>
                                    <th class="conac-cell text-center">3er Trimestre</th>
                                    <th class="conac-cell text-center">4to Trimestre</th>
...
<?php else: ?>
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-8">
                    <div class="card">
                        <div class="card-header"><i class="bi bi-bank me-1"></i> Bloques CONAC <span class="badge bg-secondary ms-1"><?= count($bloques) ?></span></div>
                        <div class="card-body p-0">
<?php if (empty($bloques)): ?>
                            <div class="text-center text-muted py-4"><i class="bi bi-bank" style="font-size:2rem"></i><p class="mt-2 mb-0">No hay bloques. Cree uno.</p></div>
<?php else: ?>
                            <div class="table-responsive">
